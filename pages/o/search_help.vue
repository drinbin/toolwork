  ...
  <div class="search_help">
    ...
    <div class="search-box">
      ...
      <div class="search-input" :class="n === 2 ? 'force' : ''">
        ...
        <a-icon ref="searchIcon" type="search" />
      </div>
    </div>
    <div class="info">
      {{ info[n - 1] }}
...
export default {
  ...
  data() {
    return {
      ...
      s: "",
      n: 0,
      px: "data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==",
      inputVal: "",
      info: [
        "第一步：找到搜索框，并点击它",
        "第二步：输入想要查找的内容",
  ...
  methods: {
    step1() {
      ...
      this.$refs.mouse.style.transform = `translateX(${
        inputXY.left + 20
      }px) translateY(${inputXY.top + 15}px) scale(-1,1)`;
    ...
    step3() {
      ...
      setTimeout(() => {
        this.n++;
        this.$refs.mouse.style.transform = `translateX(${
          searchIconXY.left - 15
        }px) translateY(${searchIconXY.top}px) scale(-1,1)`;
